...
################################################################################
# GPUMD + VASP Hybrid MD via MDI
#
# This script handles a simulation where:
#  - GPUMD runs as ENGINE (MD integrator)
#  - VASP  runs as DRIVER (QM calculator)
#
# Usage:
#   bash run_mdi_vasp_gpumd.sh [options]
#
# Options:
#   --gpumd-bin <path>        Path to GPUMD-MDI binary (default: ./gpumd-mdi)
...
log_info "==========================================================="
log_info "GPUMD + VASP Hybrid Simulation via MDI"
log_info "==========================================================="
...
log_info "Command: python3 $DRIVER_SCRIPT ${DRIVER_ARGS[*]}"

DRIVER_LOG="$TMPDIR/vasp_driver.log"
set +e
if [ "$VERBOSE" = true ]; then
    python3 "$DRIVER_SCRIPT" "${DRIVER_ARGS[@]}" | tee "$DRIVER_LOG"
    DRIVER_EXIT=${PIPESTATUS[0]}
else
    python3 "$DRIVER_SCRIPT" "${DRIVER_ARGS[@]}" > "$DRIVER_LOG" 2>&1
    DRIVER_EXIT=$?
fi
set -e
