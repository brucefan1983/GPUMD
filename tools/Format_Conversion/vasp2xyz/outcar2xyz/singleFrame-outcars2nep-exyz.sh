...
N_count=1
for i in `find -L $read_dire -name "OUTCAR" | sort -V`
do
	     configuration=$(realpath -m "$i"/.. | awk -F/ '{print $(NF-2)"/"$(NF-1)"/"$NF}')
             ...
             if grep -q "BORN EFFECTIVE CHARGES" $i
             then
                  line_properties="Properties=species:S:1:pos:R:3:forces:R:3:bec:R:9"
             else
                  line_properties="Properties=species:S:1:pos:R:3:forces:R:3"
             fi

             viri_logi=$(grep "ISIF" $i |awk '{print $3}')
             if [[ $viri_logi -ne 0 ]]
             then
                   viri=$(grep -A 20 "FORCE on cell =-STRESS" $i | grep "Total " | tail -n 1 | awk '{print $2,$5,$7,$5,$3,$6,$7,$6,$4}')
                   echo "energy=$ener Lattice=\"$latt\" virial=\"$viri\" Config_type=$configuration $line_properties" pbc=\"T T T\" >> $writ_dire/$writ_file
             else
                   echo "energy=$ener Lattice=\"$latt\" Config_type=$configuration $line_properties" pbc=\"T T T\" >> $writ_dire/$writ_file
             fi
             ...
             if grep -q "BORN EFFECTIVE CHARGES" $i
             then
                  grep -A $(($syst_numb_atom*4 + 1)) 'BORN EFFECTIVE CHARGES' $i | tail -n $(($syst_numb_atom*4)) | grep -E '^ +[1-3] ' | 
                  awk '{print $2, $3, $4}' | xargs -n 9 | awk '{for(i=1;i<=NF;i++) printf "%15.6f",$i; printf "\n"}' >> $writ_dire/bec.tem
                  paste $writ_dire/symb.tem $writ_dire/posi_forc.tem $writ_dire/bec.tem >> $writ_dire/$writ_file
             else
