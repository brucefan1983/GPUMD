...
SW1::SW1(FILE *fid, Parameters *para)
{
    ...
    sw1_para.epsilon = epsilon;
    sw1_para.lambda  = lambda;
    sw1_para.A       = A;
    sw1_para.B       = B;
    sw1_para.a       = a;
    sw1_para.gamma   = gamma;
    sw1_para.sigma   = sigma;
    sw1_para.cos0    = cos0;

    sw1_para.epsilon_times_A      = sw1_para.epsilon * sw1_para.A;
    sw1_para.epsilon_times_lambda = sw1_para.epsilon * sw1_para.lambda;
    sw1_para.sigma_times_a        = sw1_para.sigma   * sw1_para.a;

    // force cutoff
    rc = sw1_para.sigma_times_a;
    ...
    CHECK(cudaMalloc((void**)&sw1_data.f12x, memory));
    CHECK(cudaMalloc((void**)&sw1_data.f12y, memory));
    CHECK(cudaMalloc((void**)&sw1_data.f12z, memory));
}
...
SW1::~SW1(void)
{
    cudaFree(sw1_data.f12x);
    cudaFree(sw1_data.f12y);
    cudaFree(sw1_data.f12z);
}
...
static __device__ void find_p2_and_f2(SW1_Para sw, real d12, real *p2, real *f2)
{
...
(
    int number_of_particles, int pbc_x, int pbc_y, int pbc_z, SW1_Para sw,
...
void SW1::compute(Parameters *para, GPU_Data *gpu_data)
{
    int N = para->N;
    ...
    real *f12x = sw1_data.f12x; 
    real *f12y = sw1_data.f12y; 
    real *f12z = sw1_data.f12z;
           
    if (para->hac.compute)    
    {
        gpu_find_force_sw_partial<0><<<grid_size, BLOCK_SIZE_SW>>> 
        (
            N, pbc_x, pbc_y, pbc_z, sw1_para, NN, NL, x, y, z, 
            box_length, pe, f12x, f12y, f12z 
        );
        gpu_find_force_sw<0, 1, 0><<<grid_size, BLOCK_SIZE_SW>>>
        (
            N, pbc_x, pbc_y, pbc_z, sw1_para, NN, NL,
