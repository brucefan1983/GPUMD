...
Tersoff_modc::Tersoff_modc(FILE* fid, int num_of_types, const Neighbor& neighbor)
{
  ...
  for (int i = 0; i < n_entries; i++) {
    count = fscanf(
      fid, "%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf", &a, &b, &lambda, &mu, &eta,
      &delta, &alpha, &beta, &c0, &c1, &c2, &c3, &c4, &c5, &h, &r1, &r2);
    if (count != 17) {
      printf("Error: reading error for Tersoff-modc.\n");
      exit(1);
    }
...
find_fc_and_fcp(int i, const double* __restrict__ ters, double d12, double& fc, double& fcp)
{
  if (d12 < ters[i + R1]) {
    ...
    fc = NINE_OVER_16 * cos(pi_factor1 * tmp) - ONE_OVER_16 * cos(pi_factor3 * tmp) + 0.5;
...
static __device__ void find_fc(int i, const double* __restrict__ ters, double d12, double& fc)
{
  if (d12 < ters[i + R1]) {
    ...
    fc = NINE_OVER_16 * cos(ters[i + PI_FACTOR1] * tmp) -
         ONE_OVER_16 * cos(ters[i + PI_FACTOR3] * tmp) + 0.5;
  } else {
    fc = 0.0;
  }
}
...
find_g_and_gp(int i, const double* __restrict__ ters, double cos, double& g, double& gp)
{
  ...
  g = (1.0 + ters[i + C4] * exp_factor) * c2c3_factor + ters[i + C1];
  gp = ters[i + C2] * ters[i + C3] / ((ters[i + C3] + x) * (ters[i + C3] + x)) *
         (1.0 + ters[i + C4] * exp_factor) -
       ters[i + C4] * ters[i + C5] * c2c3_factor * exp_factor;
  gp *= 2.0 * (cos - ters[i + H]);
}
...
{
  double x = (cos - ters[i + H]) * (cos - ters[i + H]);
  g = (1.0 + ters[i + C4] * exp(-ters[i + C5] * x)) * ters[i + C2] * x / (ters[i + C3] + x) +
      ters[i + C1];
}
  ...
  if (n1 >= N1 && n1 < N2) {
    ...
    for (int i1 = 0; i1 < neighbor_number; ++i1) {
      ...
      if (zeta < 1.0e-16) // avoid division by 0
      {
        ...
        g_bp[i1 * number_of_particles + n1] = -b_ijj * zn * ters[ijj * NUM_PARAMS + ETA] *
  ...
  double* g_f12z)
{
  ...
  if (n1 >= N1 && n1 < N2) {
    ...
    for (int i1 = 0; i1 < neighbor_number; ++i1) {
      ...
      double factor3 =
        d12inv * (fcp_ijj_12 * (fr_ijj_12 - b12 * fa_ijj_12 + ters[ijj * NUM_PARAMS + C0]) +
      ...
      pot_energy += fc_ijj_12 * 0.5 * (fr_ijj_12 - b12 * fa_ijj_12 + ters[ijj * NUM_PARAMS + C0]);
