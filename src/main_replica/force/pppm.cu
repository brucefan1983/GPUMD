...
void PPPM::allocate_memory(const gpuStream_t stream)
{
  ...
  if (gpufftSetStream(plan, stream) != GPUFFT_SUCCESS) {
    std::cout << "GPUFFT error: Failed to bind plan to stream" << std::endl;
    exit(1);
  }
  ...
  if (need_peratom_virial) {
    ...
    if (gpufftSetStream(plan_virial, stream) != GPUFFT_SUCCESS) {
      std::cout << "GPUFFT error: Failed to bind virial plan to stream" << std::endl;
      exit(1);
    }
  }
}

void PPPM::initialize(const float alpha_input, const gpuStream_t stream)
{
  need_peratom_virial = false;
  ...
  allocate_memory(stream);
}

void PPPM::find_para(const int N, const Box& box, const gpuStream_t stream)
{
...
{
  find_para(N, box, stream);

  find_k_and_G_opt<<<(para.K0K1K2 - 1) / 64 + 1, 64, 0, stream>>>(
    para,
    kx.data(),
    ky.data(),
    kz.data(),
  ...
  set_mesh_to_zero<<<(para.K0K1K2 - 1) / 64 + 1, 64, 0, stream>>>(para, mesh.data());
  GPU_CHECK_KERNEL

  find_mesh<<<(N - 1) / 64 + 1, 64, 0, stream>>>(
    N1,
    N2,
    para,
    box,
  ...
  ik_times_mesh_times_G<<<(para.K0K1K2 - 1) / 64 + 1, 64, 0, stream>>>(
    para,
    kx.data(),
    ky.data(),
    kz.data(),
  ...
  find_mesh_G<<<(para.K0K1K2 - 1) / 64 + 1, 64, 0, stream>>>(
    para,
    G.data(),
    mesh.data(),
    mesh_G.data());
  ...
  if (need_peratom_virial) {
    find_mesh_virial<<<(para.K0K1K2 - 1) / 64 + 1, 64, 0, stream>>>(
      para,
      kx.data(),
      ky.data(),
      kz.data(),
  ...
  if (need_peratom_virial) {
    ...
    find_force_virial_potential_from_field<<<(N - 1) / 64 + 1, 64, 0, stream>>>(
      N,
      N1,
      N2,
      para,
    ...
    // get force only
    find_force_from_field<<<(N - 1) / 64 + 1, 64, 0, stream>>>(
      N1,
      N2,
      para,
      box,
    ...
    find_potential_and_virial<<<7, 1024, 0, stream>>>(
      N,
      para,
      mesh.data(),
      kx.data(),
