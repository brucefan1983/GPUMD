...
public:
  PPPM();
  ~PPPM();
  void initialize(const float alpha_input, const gpuStream_t stream);
  void find_force(
    const int N,
    const int N1,
    const int N2,
...
private:
  ...
  void allocate_memory(const gpuStream_t stream);
  void find_para(const int N, const Box& box, const gpuStream_t stream);
