...
void Ewald::find_k_and_G(const double* box, const gpuStream_t stream)
{
  float a1[3] = {0.0f};
  float a2[3] = {0.0f};
  float a3[3] = {0.0f};
  ...
  kx.copy_from_host_async(cpu_kx.data(), num_kpoints, stream);
  ky.copy_from_host_async(cpu_ky.data(), num_kpoints, stream);
  kz.copy_from_host_async(cpu_kz.data(), num_kpoints, stream);
  G.copy_from_host_async(cpu_G.data(), num_kpoints, stream);
}
...
{
  find_k_and_G(box, stream);
  find_structure_factor<<<(num_kpoints - 1) / 64 + 1, 64, 0, stream>>>(
    num_kpoints,
    N1,
    N2,
    charge.data(),
  ...
  find_force_charge_reciprocal_space<<<(N - 1) / 64 + 1, 64, 0, stream>>>(
    N,
    N1,
    N2,
    num_kpoints,
