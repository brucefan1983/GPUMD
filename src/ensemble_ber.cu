...
void Ensemble_BER::integrate_nvt_berendsen
(Parameters *para, CPU_Data *cpu_data, GPU_Data *gpu_data, Force *force)
{
    ...
    (N, temperature, temperature_coupling, thermo, vx, vy, vz);
...
void Ensemble_BER::integrate_npt_berendsen
(Parameters *para, CPU_Data *cpu_data, GPU_Data *gpu_data, Force *force)
{
    int    N           = para->N;
    int    grid_size   = (N - 1) / BLOCK_SIZE + 1;
...
(Parameters *para, CPU_Data *cpu_data, GPU_Data *gpu_data, Force *force)
{
    if (type == 1)
    {
        integrate_nvt_berendsen(para, cpu_data, gpu_data, force);
    }
    else
    {
        integrate_npt_berendsen(para, cpu_data, gpu_data, force);
    }
}
