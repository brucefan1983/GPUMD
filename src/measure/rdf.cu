  ...
  const double r_step_)
{
  ...
  double* rdf_g_ind = rdf_g_.data() + bead * rdf_atom_count * N * rdf_bins_ + rdf_atom_ * N * rdf_bins_;  

  if (rdf_atom_ == 0){
    gpu_find_rdf_ON1<<<grid_size, block_size>>>(
    N, density1[rdf_atom_], box, cell_count.data(),cell_count_sum.data(),cell_contents.data(),
    num_bins_0,num_bins_1,num_bins_2,rc_inv_cell_list, x, y, z, radial_.data(), rdf_g_ind, rdf_bins_, r_step_);
    CUDA_CHECK_KERNEL
    ...
    Atom& atom)
{
  ...
  if (is_pimd){
    ...
    for (int k = 0; k < atom.number_of_beads; k++){
      ...
      find_cell_list(
      rc_cell_list, num_bins, box, atom.position_beads[k], cell_count, cell_count_sum, cell_contents);

      for (int a = 0; a< rdf_atom_count; a++){
        find_rdf(k, rdf_atom_count, a, atom_id1_, atom_id2_,atom_id1_typesize,atom_id2_typesize,density1, density2, r_cut_, box, 
        atom.type,atom.position_beads[k],cell_count,cell_count_sum,cell_contents,num_bins[0],num_bins[1],num_bins[2],rc_inv_cell_list, radial_,rdf_g_,rdf_bins_,r_step_);
      }
    }
  }else{
    int classical = 0;
    ...
    for (int a = 0; a< rdf_atom_count; a++){
      find_rdf(classical, rdf_atom_count, a, atom_id1_,atom_id2_,atom_id1_typesize,atom_id2_typesize,density1, density2, r_cut_, box, 
    ...
    for (int k = 0; k < number_of_beads; k++){
      for (int a = 0; a< rdf_atom_count; a++){
        for (int m = 0; m < rdf_N_; m++){
          ...
          rdf_[k*num_atoms_*rdf_bins_ + a*rdf_N_*rdf_bins_ + m*rdf_bins_ + x] / num_repeat_;
          }
        }
      }
    }
