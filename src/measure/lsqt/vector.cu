  ...
  int g_offset)
{
  ...
  for (int offset = blockDim.x >> 1; offset > 0; offset >>= 1) {
    if (tid < offset) {
      s_data_real[tid] += s_data_real[tid + offset];
      s_data_imag[tid] += s_data_imag[tid + offset];
    }
    __syncthreads();
  }
  ...
  real* g_inner_product_2_imag)
{
  ...
  int number_of_batches = (number_of_blocks - 1) / BLOCK_SIZE + 1;

  for (int batch = 0; batch < number_of_batches; ++batch) {
    int n = tid + batch * BLOCK_SIZE;
    if (n < number_of_blocks) {
      int m = blockIdx.x * number_of_blocks + n;
