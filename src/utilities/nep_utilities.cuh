...
static __device__ void apply_ann_two_layers_charge(
  const int N_des,
  const int N_neu1,
  const int N_neu2,
  const float* w0,
...
{
  float x1[120], x2[120];
  float y1e[120], y2e[120];
  float y1c[120], y2c[120];
  ...
  for (int n = 0; n < N_neu2; ++n) {
    ...
    x2[n] = tanh(w1_times_x1 - b1[n]);
    energy += w2[n] * x2[n];
    charge += w2[n + N_neu2] * x2[n];
    y2e[n] = w2[n] * (1.0f - x2[n] * x2[n]);
    y2c[n] = w2[n + N_neu2] * (1.0f - x2[n] * x2[n]);
  }
  energy -= b2[0];
  ...
  for (int m = 0; m < N_neu1; ++m) {
    ...
    float sumc = 0.0f;
    for (int n = 0; n < N_neu2; ++n) {
      sume += w1[n * N_neu1 + m] * y2e[n];
      sumc += w1[n * N_neu1 + m] * y2c[n];
    }
    y1e[m] = sume * (1.0f - x1[m] * x1[m]);
    y1c[m] = sumc * (1.0f - x1[m] * x1[m]);
  }

  for (int d = 0; d < N_des; ++d) {
    for (int n = 0; n < N_neu1; ++n) {
      energy_derivative[d] += w0[n * N_des + d] * y1e[n];
      charge_derivative[d] += w0[n * N_des + d] * y1c[n];
    }
  }
}
...
static __device__ void apply_ann_two_layers_charge_vdw(
  const int N_des,
  const int N_neu1,
  const int N_neu2,
  const float* w0,
  ...
  float* C6_derivative)
{
  ...
  for (int n = 0; n < N_neu2; ++n) {
    ...
    energy += w2[n] * x2[n];
    charge += w2[n + N_neu2] * x2[n];
    C6     += w2[n + 2 * N_neu2] * x2[n];
    y2e[n]  = w2[n] * (1.0f - x2[n] * x2[n]);
    y2c[n]  = w2[n + N_neu2] * (1.0f - x2[n] * x2[n]);
    y2c6[n] = w2[n + 2 * N_neu2] * (1.0f - x2[n] * x2[n]);
  }
  energy -= b2[0];

  for (int m = 0; m < N_neu1; ++m) {
    float sume = 0.0f, sumc = 0.0f, sumc6 = 0.0f;
    for (int n = 0; n < N_neu2; ++n) {
      sume += w1[n * N_neu1 + m] * y2e[n];
      sumc += w1[n * N_neu1 + m] * y2c[n];
      sumc6 += w1[n * N_neu1 + m] * y2c6[n];
    }
    y1e[m]  = sume * (1.0f - x1[m] * x1[m]);
    y1c[m]  = sumc * (1.0f - x1[m] * x1[m]);
    y1c6[m] = sumc6 * (1.0f - x1[m] * x1[m]);
  }
  ...
  for (int d = 0; d < N_des; ++d) {
    for (int n = 0; n < N_neu1; ++n) {
      energy_derivative[d]  += w0[n * N_des + d] * y1e[n];
      charge_derivative[d]  += w0[n * N_des + d] * y1c[n];
      C6_derivative[d]      += w0[n * N_des + d] * y1c6[n];
    }
  }
}
