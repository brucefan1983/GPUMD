...
extern "C" int mdi_engine_main(struct Run* run, int argc, char* argv[])
{
  ...
  while (!exit_flag) {
    ...
    if (strcmp(command, "<NATOMS") == 0) {
      ...
      potential.resize(natoms);
      run->mdi_compute_forces();
      // Sum per-atom potential energy currently stored in GPUMD state.
      // In external-force workflows, this can be provided by >ENERGY + mdi_step_one().
      run->mdi_get_potential(potential);
      double total_energy = 0.0;
      for (int i = 0; i < natoms; ++i) {
        total_energy += potential[i];
      }
      ...
      run->mdi_set_energy(energy);
    } else if (strcmp(command, ">STRESS") == 0) {
      /* Driver sends 3x3 stress tensor (row-major, eV/A^3) */
      double stress[9];
      mdi_ret = MDI_Recv(&stress[0], 9, MDI_DOUBLE, mdi_comm);
      if (mdi_ret != 0) {
        fprintf(stderr, "ERROR: MDI_Recv failed for stress (return code %d).\n", mdi_ret);
        break;
      }
      run->mdi_set_stress(&stress[0]);
      printf("  -> Received stress from driver (eV/A^3)\n");
