...
void Ensemble_TI::init(const Atom& atom, const GPU_Vector<double>& thermo)
{
  ...
  GPU_Vector<double>& thermo)
{
  ...
  pe = thermo_cpu[1];
  espring = get_espring_sum(atom.number_of_atoms);
}
...
double Ensemble_TI::get_espring_sum(const int number_of_atoms)
{
  double temp;
