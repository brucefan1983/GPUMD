      ...
      for (int ia = 0; ia < num_cell[0]; ++ia) {
        for (int ib = 0; ib < num_cell[1]; ++ib) {
          for (int ic = 0; ic < num_cell[2]; ++ic) {
            ...
            if (distance_square < rc2) {
              NL[count * N + n1] = n2;
              g_x12[count * N + n1] = x12;
              g_y12[count * N + n1] = y12;
              g_z12[count * N + n1] = z12;
              count++;
            }
          }
        }
  ...
  if (n1 < N) {
    ...
    for (int i1 = 0; i1 < neighbor_number; ++i1) {
      ...
      float fn12[MAX_NUM_N];
        find_fn(paramb.basis_size, paramb.rcinv, d12, fc12, fn12);
  ...
  if (n1 < N) {
    ...
    for (int n = 0; n <= paramb.n_max; ++n) {
      ...
      if (paramb.num_L == paramb.L_max) {
        find_q(paramb.n_max + 1, n, s, q);
      } else if (paramb.num_L == paramb.L_max + 1) {
        find_q_with_4body(paramb.n_max + 1, n, s, q);
      } else {
        find_q_with_5body(paramb.n_max + 1, n, s, q);
      }
    ...
    for (int n = 0; n <= paramb.n_max; ++n) {
      for (int l = 0; l < paramb.num_L; ++l) {
        int ln = l * (paramb.n_max + 1) + n;
        g_descriptors[n1 + ((paramb.n_max + 1) + ln) * N] = q[ln];
      }
    }
  }
}
  ...
  int deviceCount)
{
  ...
  paramb.dim_angular = (paramb.n_max + 1) * paramb.num_L;

  paramb.basis_size = para.basis_size_radial;
  ...
  for (int device_id = 0; device_id < deviceCount; device_id++) {
    ...
    nep_data[device_id].NN.resize(N);
    nep_data[device_id].NL.resize(N_times_max_NN);
    nep_data[device_id].x12.resize(N_times_max_NN);
    nep_data[device_id].y12.resize(N_times_max_NN);
    nep_data[device_id].z12.resize(N_times_max_NN);
    nep_data[device_id].descriptors.resize(N * annmb[device_id].dim);
    nep_data[device_id].Fp.resize(N * annmb[device_id].dim);
    nep_data[device_id].sum_fxyz.resize(N * (paramb.n_max + 1) * NUM_OF_ABC);
  ...
  if (n1 < N) {
    ...
    for (int i1 = 0; i1 < neighbor_number; ++i1) {
      ...
      float f12[3] = {0.0f};
        ...
        find_fn_and_fnp(
          paramb.basis_size, paramb.rcinv, d12, fc12, fcp12, fn12, fnp12);
        for (int n = 0; n <= paramb.n_max; ++n) {
          float gnp12 = 0.0f;
          for (int k = 0; k <= paramb.basis_size; ++k) {
            int c_index = (n * (paramb.basis_size + 1) + k) * paramb.num_types_sq;
  ...
  float* g_virial)
{
  ...
  if (n1 < N) {
    ...
    for (int d = 0; d < paramb.dim_angular; ++d) {
      Fp[d] = g_Fp[(paramb.n_max + 1 + d) * N + n1];
    }
    for (int d = 0; d < (paramb.n_max + 1) * NUM_OF_ABC; ++d) {
    ...
    for (int i1 = 0; i1 < neighbor_number; ++i1) {
      ...
      float f12[3] = {0.0f};
        ...
        for (int n = 0; n <= paramb.n_max; ++n) {
          ...
          if (paramb.num_L == paramb.L_max) {
            accumulate_f12(n, paramb.n_max + 1, d12, r12, gn12, gnp12, Fp, sum_fxyz, f12);
          } else if (paramb.num_L == paramb.L_max + 1) {
            accumulate_f12_with_4body(
              n, paramb.n_max + 1, d12, r12, gn12, gnp12, Fp, sum_fxyz, f12);
          } else {
  ...
  for (int device_id = 0; device_id < device_in_this_iter; ++device_id) {
    ...
    if (calculate_neighbor) {
      gpu_find_neighbor_list<<<dataset[device_id].Nc, 256>>>(
        ...
        nep_data[device_id].NN.data(),
        nep_data[device_id].NL.data(),
        nep_data[device_id].x12.data(),
        nep_data[device_id].y12.data(),
        nep_data[device_id].z12.data());
      CUDA_CHECK_KERNEL
    }
