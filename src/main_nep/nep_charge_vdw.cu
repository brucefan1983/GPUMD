...
static __global__ void gpu_find_neighbor_list(
  const NEP_ChargeVdW::ParaMB paramb,
  const int N,
  const int* Na,
  const int* Na_sum,
...
void NEP_ChargeVdW::update_potential(Parameters& para, float* parameters, ANN& ann)
{
  ...
  if (n1 < N) {
    ...
    for (int i1 = 0; i1 < neighbor_number; ++i1) {
      ...
      for (int n = 0; n <= paramb.n_max_radial; ++n) {
        ...
        const float tmp12 = (g_Fp[n1 + n * N] + g_charge_derivative[n1 + n * N] * g_D_real[n1] 
          + g_C6_derivative[n1 + n * N] * g_D_C6[n1]) * gnp12 * d12inv;
        for (int d = 0; d < 3; ++d) {
          f12[d] += tmp12 * r12[d];
        }
      }
  ...
  if (n1 < N) {
    ...
    for (int d = 0; d < paramb.dim_angular; ++d) {
      Fp[d] = g_Fp[(paramb.n_max_radial + 1 + d) * N + n1] 
        ...
        + g_C6_derivative[(paramb.n_max_radial + 1 + d) * N + n1] * g_D_C6[n1];
    }
