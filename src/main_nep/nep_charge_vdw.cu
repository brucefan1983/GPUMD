...
static __global__ void find_descriptors_radial(
  ...
  const NEP_Charge_VDW::ANN annmb,
  ...
  int deviceCount)
{
  ...
  for (int t = 0; t < paramb.num_types; ++t) {
    ...
    if (z < 1 || z > max_elem_vdw) {
      PRINT_INPUT_ERROR("NEP-charge-vdW currently supports elements from H to Pu.");
    }
  ...
  charge_para.alpha = float(PI) / para.rc_radial_max; // a good value
  ...
  if (para.nep_compile && para.prediction == 0) {
    ...
    compiled_kernel_.reset(new NEP_Compile(
      make_nep_compile_config(para, NEP_Compile_Mode::CHARGE_VDW, paramb.c6_ref_sqrt)));
...
void NEP_Charge_VDW::update_potential(float* parameters, ANN& ann)
{
  ...
  for (int batch = 0; batch < number_of_batches; ++batch) {
    ...
    if (n < N2) {
      ...
      for (int nk = 0; nk < num_kpoints; ++nk) {
        ...
        if (ksq == 0.0f) {
          continue; // k=0 belongs to vdW only
        }
      ...
      g_pe[n] += K_C_SP *
        (temp_energy_sum / (N2 - N1) -
         0.5f * q1 * q1 * two_alpha_over_sqrt_pi);
      for (int d = 0; d < 6; ++d) {
        g_virial[n + N * d] += K_C_SP * temp_virial_sum[d] / (N2 - N1);
      }
      g_D_real[n] = K_C_SP *
        (2.0f * temp_D_real_sum - q1 * two_alpha_over_sqrt_pi);
      const float charge_factor = K_C_SP * 2.0f * q1;
          ...
          if (ksq < ksq_max) {
            if (g_kx != nullptr) {
              ...
              if (ksq == 0.0f) {
                ...
                g_G[nc_nk] =
                  2.0f * abs(two_pi_over_det) / ksq *
                  exp(-ksq * alpha_factor);
...
void NEP_Charge_VDW::prepare_kpoints(Dataset& dataset, int device_id)
{
  NEP_Charge_VDW_Data& data = nep_data[device_id];
...
void NEP_Charge_VDW::find_force(
  Parameters& para,
  ...
  int device_in_this_iter)
{
  ...
  for (int device_id = 0; device_id < device_in_this_iter; ++device_id) {
    ...
    if (compiled_kernel_) {
      compiled_kernel_->launch_ann_charge_vdw(
        dataset[device_id].N,
        dataset[device_id].type.data(),
        nep_data[device_id].descriptors.data(),
        para.q_scaler_gpu[device_id].data(),
    ...
    if (compiled_kernel_) {
      compiled_kernel_->launch_force_charge_vdw_radial(
        dataset[device_id].N,
        dataset[device_id].NN_radial_sum.data(),
        dataset[device_id].NN_radial.data(),
        dataset[device_id].NL_radial.data(),
    ...
    if (compiled_kernel_) {
      compiled_kernel_->launch_force_charge_vdw_angular(
        dataset[device_id].N,
        dataset[device_id].NN_angular_sum.data(),
        dataset[device_id].NN_angular.data(),
        dataset[device_id].NL_angular.data(),
