    ...
    if (has_test_set) {
      if (para.train_mode == 0) {
        ...
        update_energy_force_virial(
          fid_energy, fid_force, fid_virial, test_set[0], calc_stored, ref_stored);
        for (int ii = 0; ii < calc_stored[0].size(); ii++) {
          fprintf(
            fid_force, "%g %g %g %g %g %g\n", calc_stored[0][ii], calc_stored[1][ii],
            calc_stored[2][ii], ref_stored[0][ii], ref_stored[1][ii], ref_stored[2][ii]);
        }
...
void Fitness::update_energy_force_virial(
  ...
  FILE* fid_virial,
  Dataset& dataset,
  std::vector<std::vector<double>>& calc_stored,
  std::vector<std::vector<double>>& ref_stored)
{
  std::vector<double> calc_ret;
  std::vector<double> ref_ret;
  ...
  predict_energy_or_stress(
    fid_energy, dataset.energy_cpu.data(), dataset.energy_ref_cpu.data(), dataset, &calc_ret,
    &ref_ret);
...
void Fitness::predict(Parameters& para, float* elite)
{
  if (para.train_mode == 0) {
    ...
    for (int batch_id = 0; batch_id < num_batches; ++batch_id) {
      potential->find_force(para, elite, train_set[batch_id], false, true, 1);
      update_energy_force_virial(
        fid_energy, fid_force, fid_virial, train_set[batch_id][0], calc_stored, ref_stored);
    }
