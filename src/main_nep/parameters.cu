...
void Parameters::set_default_parameters()
{
  ...
  num_neurons[1] = 0;          // default is to use a single hidden layer
...
void Parameters::calculate_parameters()
{
  ...
  if (num_neurons[1] == 0) {
    number_of_variables_one_ann_without_bias = (dim + 2) * num_neurons[0];
  } else {
    number_of_variables_one_ann_without_bias =
      (dim + 1) * num_neurons[0] + (num_neurons[0] + 2) * num_neurons[1];
  }
  number_of_variables_ann =
    number_of_variables_one_ann_without_bias * (version == 4 ? num_types : 1) + 1;
...
void Parameters::report_inputs()
{
  ...
  if (is_neuron_set) {
    printf("    (input)   number of neurons = (%d, %d).\n", num_neurons[0], num_neurons[1]);
  } else {
    printf("    (default) number of neurons = (%d, %d).\n", num_neurons[0], num_neurons[1]);
  }
