...
)
{
  ...
  const float per_Nc = 2.0f * lambda_f / N / 3;
  ...
  for (int c_idx = start_var + tid; c_idx < end_var; c_idx += blockDim.x) {
    ...
    for (int n = N1; n < N2; ++n) {
      ...
      grad_sum += (g_F_x_grad[c_idx * N + n] * dx +
                   g_F_y_grad[c_idx * N + n] * dy +
                   g_F_z_grad[c_idx * N + n] * dz);
    }
...
static __global__ void gpu_sum_pe_error(
  float energy_shift, int* g_Na, int* g_Na_sum, float* g_pe, float* g_pe_ref, float* diff_gpu, float* error_gpu)
{
  int tid = threadIdx.x;
  int bid = blockIdx.x;
...
)
{
  ...
  const float per_Nc = 2.0f * lambda_e / Nc;

  for (int c_idx = start_var + tid; c_idx < end_var; c_idx += blockDim.x) {
    if (c_idx >= num_var) continue;
    float temp = 0.0f;
    for (int n = N1; n < N2; ++n) {
      temp += g_E_grad[c_idx * N + n];
    }
...
)
{
  ...
  for (int c_idx = start_var + tid; c_idx < end_var; c_idx += blockDim.x) {
    ...
    for (int n = N1; n < N2; ++n) {
      V_xx += g_V_xx[c_idx * N + n];
      V_yy += g_V_yy[c_idx * N + n];
      V_zz += g_V_zz[c_idx * N + n];
      V_xy += g_V_xy[c_idx * N + n];
      V_yz += g_V_yz[c_idx * N + n];
      V_zx += g_V_zx[c_idx * N + n];
    }
