...
PotentialCavity::PotentialCavity(void) { rc = 0.0; }

PotentialCavity::~PotentialCavity(void)
{
  // nothing
}
...
void PotentialCavity::find_properties_many_body(
  Box& box,
  const int* NN,
  const int* NL,
  const double* f12x,
  ...
  double* g_virial)
{
  ...
  if (n1 >= N1 && n1 < N2) {
    ...
    for (int i1 = 0; i1 < neighbor_number; ++i1) {
      ...
      // per-atom virial
      if (is_dipole) {
        // double version of the function
        // The dipole is proportional to minus the sum of the virials times r12
        double r12_square = x12 * x12 + y12 * y12 + z12 * z12;
