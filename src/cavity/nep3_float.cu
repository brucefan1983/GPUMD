...
#include "cavity/nep3_float.cuh"
#include "cavity/nep3_small_box_float.cuh"
...
NEP3Float::NEP3Float(const char* file_potential, const int num_atoms)
{
...
NEP3Float::~NEP3Float(void)
{
  // nothing
}

void NEP3Float::update_potential(float* parameters, ANN& ann)
{
  float* pointer = parameters;
  for (int t = 0; t < paramb.num_types; ++t) {
    if (t > 0 && paramb.version != 4) { // Use the same set of NN parameters for NEP2 and NEP3Float
...
#ifdef USE_TABLE
void NEP3Float::construct_table(float* parameters)
{
...
void NEP3Float::compute_large_box(
  Box& box,
...
void NEP3Float::compute_small_box(
  Box& box,
  const GPU_Vector<int>& type,
  const GPU_Vector<double>& position_per_atom,
  GPU_Vector<double>& potential_per_atom,
...
static bool get_expanded_box(const double rc, const Box& box, NEP3Float::ExpandedBox& ebox)
{
...
static bool get_expanded_box_jacobian(const double rc, const Box& box, NEP3Float::ExpandedBox& ebox)
{
  double volume = box.get_volume();
  double thickness_x = volume / box.get_area(0);
  double thickness_y = volume / box.get_area(1);
...
void NEP3Float::compute_jacobian(
  Box& box,
