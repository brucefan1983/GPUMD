...
NEP3Cavity::NEP3Cavity(const char* file_potential, const int num_atoms)
{
  ...
  std::vector<double> parameters(annmb.num_para);
...
NEP3Cavity::~NEP3Cavity(void)
{
  // nothing
}

void NEP3Cavity::update_potential(double* parameters, ANN& ann)
{
  double* pointer = parameters;
  for (int t = 0; t < paramb.num_types; ++t) {
    if (t > 0 && paramb.version != 4) { // Use the same set of NN parameters for NEP2 and NEP3Cavity
...
#ifdef USE_TABLE
void NEP3Cavity::construct_table(double* parameters)
{
  ...
  std::vector<double> gn_radial(table_length * paramb.num_types_sq * (paramb.n_max_radial + 1));
  std::vector<double> gnp_radial(table_length * paramb.num_types_sq * (paramb.n_max_radial + 1));
  std::vector<double> gn_angular(table_length * paramb.num_types_sq * (paramb.n_max_angular + 1));
  std::vector<double> gnp_angular(table_length * paramb.num_types_sq * (paramb.n_max_angular + 1));
  double* c_pointer =
    parameters +
    (annmb.dim + 2) * annmb.num_neurons1 * (paramb.version == 4 ? paramb.num_types : 1) + 1;
...
#ifdef USE_TABLE
  const double* __restrict__ g_gn_radial,
  const double* __restrict__ g_gn_angular,
#endif
  double* g_pe,
  double* g_Fp,
  double* g_virial,
...
#ifdef USE_TABLE
  const double* __restrict__ g_gnp_radial,
#endif
  double* g_fx,
  double* g_fy,
  double* g_fz,
  double* g_virial)
{
  ...
  if (n1 < N2) {
    ...
    for (int i1 = 0; i1 < g_NN[n1]; ++i1) {
      ...
      double z12double = g_z[n2] - z1;
      apply_mic(box, x12double, y12double, z12double);
      double r12[3] = {double(x12double), double(y12double), double(z12double)};
      double d12 = sqrt(r12[0] * r12[0] + r12[1] * r12[1] + r12[2] * r12[2]);
      double d12inv = 1.0f / d12;
      double f12[3] = {0.0f};
      double f21[3] = {0.0f};
#ifdef USE_TABLE
      int index_left, index_right;
      ...
      for (int n = 0; n <= paramb.n_max_radial; ++n) {
        ...
        double tmp21 = g_Fp[n2 + n * N] * gnp21 * d12inv;
        for (int d = 0; d < 3; ++d) {
          f12[d] += tmp12 * r12[d];
          f21[d] -= tmp21 * r12[d];
        }
      ...
      if (paramb.version == 2) {
        ...
        for (int n = 0; n <= paramb.n_max_radial; ++n) {
          ...
          double tmp21 = g_Fp[n2 + n * N] * fnp12[n] * d12inv;
...
#ifdef USE_TABLE
  const double* __restrict__ g_gn_angular,
  const double* __restrict__ g_gnp_angular,
#endif
  double* g_f12x,
  double* g_f12y,
  double* g_f12z)
{
  ...
  if (n1 < N2) {
    ...
    double Fp[MAX_DIM_ANGULAR] = {0.0f};
...
void NEP3Cavity::compute_large_box(
  Box& box,
...
void NEP3Cavity::compute_small_box(
  Box& box,
  const GPU_Vector<int>& type,
  const GPU_Vector<double>& position_per_atom,
  GPU_Vector<double>& potential_per_atom,
  ...
  GPU_Vector<double>& virial_per_atom)
{
  ...
  GPU_Vector<double> r12(size_x12 * 6);
...
static bool get_expanded_box(const double rc, const Box& box, NEP3Cavity::ExpandedBox& ebox)
{
