...
#ifndef MOS2_JIANG
template <int cal_p>
static __global__ void gpu_find_force_sw2_partial 
(
...
void gpu_find_force_sw2(Parameters *para, SW2 sw2, GPU_Data *gpu_data)
{
    ...
    if (para->hac.compute)    
    {
        gpu_find_force_sw2_partial<0><<<grid_size, BLOCK_SIZE_SW>>> 
        (
            N, pbc_x, pbc_y, pbc_z, sw2, NN, NL, type, x, y, z, 
            box_length, pe, f12x, f12y, f12z 
        );

        gpu_find_force_sw2<0, 1, 0><<<grid_size, BLOCK_SIZE_SW>>>
        (
            N, pbc_x, pbc_y, pbc_z, sw2, NN, NL, type, 
    ...
    {
        gpu_find_force_sw2_partial<1><<<grid_size, BLOCK_SIZE_SW>>> 
        (
            N, pbc_x, pbc_y, pbc_z, sw2, NN, NL, type, x, y, z, 
            box_length, pe, f12x, f12y, f12z 
        );
