...
FIXED NUMBER OF STEPS the accumulated (global) energy drift scales as O(time_step**2). The drift
bound below is expressed as ENERGY_DRIFT_COEFFICIENT * TIME_STEP**2 (eV/fs**2 * fs**2 = eV)
rather than a bare constant, so it's explicit that halving TIME_STEP should relax this bound 4x
if this scaling assumption holds. `velocity <T>` zeros total linear/angular momentum at
initialization (see doc/gpumd/input_parameters/velocity.rst), and NVE has no external
forces/torques, so momentum should stay at ~0 throughout; MOMENTUM_DRIFT_BOUND is an absolute
bound on the largest total-momentum norm observed over the trajectory, not time_step-scaled,
since momentum conservation is exact at the equations-of-motion level regardless of step size --
any drift here is purely numerical.
"""
import numpy as np
import pytest
...
def _run_nve(tmp_path, structure, model_path, gpumd_command):
    ...
    params = [
        ...
        # velocity draw, especially for the 3-atom water_molecule fixture, so an unseeded run
...
def test_nve_conservation(tmp_path, structure, model_path, gpumd_command):
    ...
    energy_bound = ENERGY_DRIFT_COEFFICIENT * TIME_STEP**2
