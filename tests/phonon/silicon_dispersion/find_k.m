...
NK=Nk*(num_of_special_k-1);
K=zeros(3,NK);
for n=1:num_of_special_k-1
    for d=1:3
       K(d,(n-1)*Nk+1:n*Nk)=linspace(special_k(d,n),special_k(d,n+1),Nk);
    end
end

k_distance=zeros(num_of_special_k,1);
k_distance(1)=0;
for n=1:num_of_special_k-1
    k_distance(n+1)=norm(special_k(:,n+1)-special_k(:,n));
end
