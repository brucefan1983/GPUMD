...
\item Implemented the HNEMD method \cite{fan2018submitted,xu2018submitted,dong2018submitted} for all the potentials.
\end{enumerate}
...
The current version of GPUMD has 57 source files and about 14 000 lines of code. Most files come in pairs. For example, \verb"gpumd" below means \verb"gpumd.cu" and \verb"gpumd.cuh". If a file does not come in pair, we write the suffix explicitly. These only include \verb"main.cu", \verb"mic.inc",  and \verb"ensemble.inc", and the last two will be directly included into some other files. In the figure, if there is an arrow pointing from file A to file B, it means that file A will call some functions in file B or the class defined in file A is inherited from the class defined in file B. Nearly all the files depend on \verb"common", as indicated by the many arrows pointing to it.
...
\item column 6: $\langle J_z^{\text{tot}}(0)J_z^{\text{tot}}(t)\rangle$ (in unit of eV$^3$/amu)
...
\item column 11: $\kappa_z^{\text{tot}}(t)$ (in unit of Wm$^{-1}$K$^{-1}$)
\end{itemize}
