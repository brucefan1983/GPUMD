...
\begin{equation}
b_{ij} =b_{ij}^{\sigma\pi} + b_{ij}^{\rm RC} + b_{ij}^{\rm DH}.
\end{equation}
...
where
\begin{equation}
\zeta_{ij} = \left[ \sum_{k\neq i, j} f_C(r_{ik}) g_{ijk}(\cos\theta_{ijk}, N_{i}) \right] + P_{ij}(N_{i}^{\rm C}, N_{i}^{\rm H});
\end{equation}
\begin{equation}
N_{i}^{C} = \sum_{k\neq i}^{\rm{C~atoms}} f_C(r_{ik});
\end{equation}
\begin{equation}
N_{i}^{H} = \sum_{k\neq i}^{\rm{H~atoms}} f_C(r_{ik}).
\end{equation}
\begin{equation}
N_{i} =N_{i}^{C}+N_{i}^{H}.
\end{equation}

The function $P(x,y)$ is given as a cubic spline. TODO:  figure out the details.

The angular function $g(x,y)$  is also given as a spline. TODO: figure out the details.

TODO: Derive the partial force related to the $\sigma\pi$ bonding part.

The RC part is given as
\begin{equation}
b_{ij}^{\rm RC} = F_{ij}(N_i,N_j,N_{ij}^{\rm conj})
\end{equation}
where $F_{ij}(x,y,z)$ is a spline.  $N_{ij}^{\rm conj}$ is kind of complicated.

TODO: Derive the partial force related to the RC part.

The DH part is given as
\begin{equation}
b_{ij}^{\rm DH} =T_{ij}(N_i,N_j,N_{ij}^{\rm conj}) 
\sum_{k\neq i,j}\sum_{l\neq i,j} f_{C}(r_{ik}) f_{C}(r_{jl})\left(1-\cos^2\Theta_{ijkl}\right)
\end{equation}
where $T_{ij}(x,y,z)$ is a spline, and $\Theta_{ijkl}$ is the dihedral angle formed by the $ijk$ plane and the $ijl$ plane.

TODO: Derive the partial force related to the DH part.
