...
\end{verbatim}
When the compilation finishes, an executable named \verb"phonon" will be generated in the \verb"src" directory.

The compiling flag in \verb"makefle.phonon" for building \verb"phonon" is similar to that for \verb"gpumd" . However, there is an extra linking flag:
\begin{verbatim}
...
\section{Inputs}
There are three mandatory input files:
\begin{itemize}
\item \verb"phonon.in": the controlling file
...
\subsubsection{phonon.in}

This file is similar to the run.in file for gpumd. It only has three keywords now:
\begin{itemize}
...
Here,
\begin{itemize}
\item \verb"N_basis" is the number of atoms in the primitive cell. For example, it can be 2 for diamond silicon. The next \verb"N_basis" lines contains the indices and masses for the basis atoms.
\item \verb"id(1)" and \verb"mass(1)" are the atom index and mass of the first basis atom in the primitive cell. For the example we provided, \verb"id(1)=0" \verb"mass(1)=28", \verb"id(2)=4" and \verb"mass(2)=28".
\item The remaining lines map the atoms to the basis atoms. If an atom is equivalent (under translation) to the $n$-th basis atom in the primitive cell, give it a number of $n$. In our example, it is either 0 (equivalent to the first basis atom) or 1 (equivalent to the second basis atom).
\end{itemize}

\subsubsection{kpoints.in}
The format of this file must be as follows:
\begin{verbatim}
N_kpoints
kx(1) ky(1) kz(1)
kx(1) ky(1) kz(1)
...
kx(N_kpoints) ky(N_kpoints) kz(N_kpoints)
\end{verbatim}
\section{Outputs}
