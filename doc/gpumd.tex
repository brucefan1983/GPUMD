...
The current version of GPUMD has 59 source files. Except for a few files ( \verb"main.cu", \verb"mic.cu", \verb"mic_template.cu"), all the other files come in pairs (e.g., \verb"gpumd" below means \verb"gpumd.cu" and \verb"gpumd.h"). The logical dependence of the files can be summarized as follows:
...
V_2(r_{ij}) =  A \epsilon
\left[
  B \left( \frac{\sigma_{ij} }{ r_{ij} } \right)^{4}-1
\right]
...
\exp
\left[
\frac{\gamma}{r_{ij}/\sigma_{ij}-a_{ij}} + \frac{\gamma}{r_{ik}/\sigma_{ij}-a_{ij}}
\right]
...
\frac{\partial U_i}{\partial \vect{r}_{ij}}
&= \lambda \exp \left[
 \frac{\gamma}{r_{ij}/\sigma_{ij}-a_{ij}} + \frac{\gamma}{r_{ik}/\sigma_{ik}-a_{ik}}
...
\section{The REBO potential for Mo-S systems by Liang \textit{et al.}}
...
% Something to be added in a future version...
