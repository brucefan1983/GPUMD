...
This is version 1.1, which has the following major changes compared to version 1.0:
\begin{enumerate}
...
The current version of GPUMD has 55 source files. Except for the \verb"main.cu" file, all the other files come in pairs (e.g., \verb"gpumd" below means \verb"gpumd.cu" and \verb"gpumd.h"). The logical dependence of the files can be summarized as follows:
...
\section{The modified Stillinger-Weber potential for single-layer black phosphorene}

In version 1.1, we added a modified Stillinger-Weber potential model for single-layer black phosphorene (SLBP) as introduced by Xu \textit{et al.} \cite{xu2015jap}. SLBP is not strictly flat. The phosphorous atoms in SLBP occupy two planes and the potential model by Xu \textit{et al.} \cite{xu2015jap} distinguishes atoms in different planes. Therefore, this potential model assumes the topology of SLBP and should not be used to study extreme conditions such as melting. In GPUMD, one has to define the atoms in the two planes as types 0 and 1 when preparing the \verb"xyz.in" file to be introduced in Chapter \ref{chapter:usage}.
...
% Something to be added in future version...

\subsection{The EDIP potential}
...
\end{equation}
\begin{equation}
U_i^{(3)} = \frac{1}{2}\sum_{j\neq i}\sum_{k\neq i,j}  V^{(3)}_{ijk} \left(r_{ij}, r_{ik}, Z_i \right).
\end{equation}
...
\begin{align}
h \left(\cos \theta_{ijk} , Z_i \right) &=
\lambda \left[1 - \exp \left( -Q_0 e^{-\mu Z} (\cos \theta_{ijk} + \tau(Z))^2 \right) \right] \nonumber \\
&+ \lambda \eta Q_0 e^{-\mu Z} (\cos \theta_{ijk} + \tau(Z))^2.
\end{align}
where
\begin{equation}
\tau(Z) = 0.165799 + 32.557(0.286198 e^{-0.66 Z} - e^{-1.32 Z} ).
\end{equation}
...
&+ \frac{1}{2} \frac{\partial V^{(2)}_{ij}}{\partial Z_{i}} f'_{ij}(r_{ij}) \frac{\partial r_{ij}}{\partial \vect{r}_{ij}} \nonumber \\
...
&= \sum_{k\neq i,j}
\left( \frac{\partial V^{(3)}_{ijk}}{\partial r_{ij}} + \frac{\partial V^{(3)}_{ijk}}{\partial Z_{i}} f'_{ij}(r_{ij}) \right)
\frac{\partial r_{ij}}{\partial \vect{r}_{ij}} \nonumber \\
&+ \frac{1}{2} \sum_{k\neq i,j} \sum_{m\neq i,j} \frac{\partial V^{(3)}_{ikm}}{\partial Z_{i}} f'_{ij}(r_{ij}) \frac{\partial r_{ij}}{\partial \vect{r}_{ij}}.
\end{align}
...
\subsection{Modified Tersoff potential by Kumagai \textit{et al.}}

A modified angular function has been proposed by Kumagai \textit{et al.} \cite{kumagai2007cms}.
The bond-order is takes a more general form
\begin{equation}
b_{ij} = \left(1 + \zeta_{ij}\right)^{-\delta},
\end{equation}
where
\begin{equation}
\zeta_{ij} = \sum_{k\neq i, j}f_C(r_{ik}) g_{ijk} e^{\alpha (r_{ij} - r_{ik})},
\end{equation}
and
\begin{equation}
g_{ijk} = c_1 + \frac{c_2(h-\cos\theta)^2}{c3+(h-\cos\theta)^2}
                \left[1+e^{-c_5(h-\cos\theta)^2}\right].
\end{equation}
They also adopt the modified cutoff function proposed by
Mutrty, which has a continuous second-order derivative:
\begin{equation}
f_{C}(r) = \frac{1}{2} +
\frac{9}{16} \cos \left( \pi \frac{r_{ij} - r_{ij}^{min}}{r_{ij}^{max} - r_{ij}^{min}} \right) -
\frac{1}{16} \cos \left( 3\pi \frac{r_{ij} - r_{ij}^{min}}{r_{ij}^{max} - r_{ij}^{min}} \right)
\end{equation}

The parameters are $A$, $B$, $\lambda_1$, $\lambda_2$, $\delta$, $\alpha$, $c_1$, $c_2$, $c_3$, $c_5$ and can be found from Table 7 of Kumagai \textit{et al.}.
...
&= \frac{1}{2}f_C'(r_{ij})[f_R(r_{ij})-b_{ij}f_A(r_{ij})]\frac{\partial r_{ij}}{\partial \vect{r}_{ij}} \nonumber \\
&+ \frac{1}{2}f_C(r_{ij})[f_R'(r_{ij})-b_{ij}f_A'(r_{ij})]\frac{\partial r_{ij}}{\partial \vect{r}_{ij}} \nonumber \\
&- \frac{1}{2}f_C'(r_{ij})\sum_{k\neq i,j}f_C(r_{ik})f_A(r_{ik})b'_{ik}g_{ijk} e^{\alpha (r_{ik}-r_{ij})}\frac{\partial r_{ij}}{\partial \vect{r}_{ij}} \nonumber \\
&- \frac{1}{2}f_C(r_{ij})f_A(r_{ij})b'_{ij}\sum_{k\neq i,j}f_C(r_{ik}) e^{\alpha (r_{ij}-r_{ik})} g'_{ijk}
   \frac{\partial \cos\theta_{ijk}}{\partial \vect{r}_{ij}} \nonumber \\
&- \frac{1}{2}f_C(r_{ij})f_A(r_{ij})b'_{ij}\sum_{k\neq i,j}f_C(r_{ik}) g_{ijk} \alpha e^{\alpha (r_{ij}-r_{ik})}
   \frac{\partial r_{ij}}{\partial \vect{r}_{ij}} \nonumber \\
&- \frac{1}{2}f_C(r_{ij})\sum_{k\neq i,j}f_C(r_{ik})f_A(r_{ik})b'_{ik} e^{\alpha (r_{ik}-r_{ij})} g'_{ijk}
   \frac{\partial \cos\theta_{ijk}}{\partial \vect{r}_{ij}}. \nonumber \\
&- \frac{1}{2}f_C(r_{ij})\sum_{k\neq i,j}f_C(r_{ik})f_A(r_{ik})b'_{ik} (-\alpha) e^{\alpha (r_{ik}-r_{ij})}
...
\subsection{The REBO potential for Mo-S systems by Liang \textit{et al.}}


The erratum \cite{liang2012prb_erratum} should be consulted for improved and extra model parameters which are missing from the original article.
...
U_i =  \frac{1}{2} \sum_{j \neq i} f_C(r_{ij}) \left[ f_R(r_{ij}) - b_{ij} f_A(r_{ij}) \right]
\end{equation}
The cutoff function is identical to that in the Tersoff potential.
\begin{equation}
f_{R}(r) = \left(1+\frac{Q}{r_{ij}}\right) A e^{-\alpha r_{ij}};
\end{equation}
\begin{equation}
f_{A}(r) = B e^{-\beta r_{ij}}.
\end{equation}
...
where
\begin{equation}
\zeta_{ij} =\sum_{k\neq i, j} f_C(r_{ik}) g(\cos\theta_{ijk}) + P(N_{ij});
\end{equation}
\begin{equation}
N_{ij} = \sum_{k\neq i,j} f_C(r_{ik});
\end{equation}
\begin{equation}
P(x) = -a_0(x-1) - a_1 e^{-a_2x}+a3.
\end{equation}
The angular function $g(x)$ is
\begin{equation}
g(x)= G(x) + \psi(x) [ \gamma(x) - G(x)]
\end{equation}
where both $G(x)$ and $\gamma(x)$ are 6-th order polynomials and
\begin{equation}
...
\end{equation}
The parameters can be found the original article and the erratum.
