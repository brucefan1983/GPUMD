...
The current version of GPUMD has about 15 000 lines of code. Most files come in pairs. For example, \verb"gpumd" below means \verb"gpumd.cu" and \verb"gpumd.cuh". If a file does not come in pair, we write the suffix explicitly. In the figure, if there is an arrow pointing from file A to file B, it means that file A will call some functions in file B or the class defined in file A is inherited from the class defined in file B. Nearly all the files depend on \verb"common", as indicated by the many arrows pointing to it. Not all the files are in the figure.
...
\textcolor{red}{Warning: The current implementation only applies to situations in which each atom in group $A$ only has at most one neighbor atom in group $B$. We are working on a more general implementation.}
...
\item column 2: total energy (in units of eV) of the system if the thermostat method is Nos\'{e}-Hoover chain and kinetic energy (in unit of eV) of the system otherwise
\item column 3: total energy (in units of eV) of the thermostat if the thermostat method is Nos\'{e}-Hoover chain and potential energy (in unit of eV) of the system otherwise
