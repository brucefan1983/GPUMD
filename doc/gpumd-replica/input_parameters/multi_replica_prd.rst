...
::
  ...
  multi_replica prd replicas <number> [<per_gpu>] event <interval> dephase <iterations> <steps> correlate <steps> distance <threshold> [quench <force_tolerance> <max_steps>] [max_dephase_retries <number>] [resume <prefix>]
...
* ``dephase``: nonnegative iteration count and positive MD steps per iteration.
  Escaping trials are restored and retried.
...
* ``max_dephase_retries``: positive trial-round limit per dephasing iteration; default 1000.
...
::
  ...
  multi_replica prd replicas 8 event 10 dephase 5 1000 correlate 1000 distance 0.5
