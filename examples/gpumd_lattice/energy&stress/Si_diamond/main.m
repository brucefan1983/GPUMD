...
% panel (a): force
subplot(1,3,1)
plot(f_tersoff,force(513:end,1:3),'o');
hold on 
x=-2:0.5:2;
plot(x,x,'b-')
xlim([-2,2]);
...
% panel (b): energy
subplot(1,3,2);
plot(a,energy_MD(1:end/3,2),'d');
hold on;
plot(a,energy(1:end/3,1),'-','linewidth',2);
plot(a,energy_MD(end/3+1:2*end/3,2),'s');
plot(a,energy(end/3+1:2*end/3,1),'--','linewidth',2);
plot(a,energy_MD(2*end/3+1:3*end/3,2),'o');
...
set(gca,'fontsize',12,'ticklength',get(gca,'ticklength')*2);
legend('triaxial-MD','triaxial-GA','biaxial-MD',...
   'biaxial-GA','uniaxial-MD','uniaxial-GA');
title('(b)');

% panel (c): virial in a deformed direction
subplot(1,3,3);
plot(a,virial_MD(1:end/3,2),'d','linewidth',1);
hold on;
plot(a,virial_xx(1:end/3,1),'-','linewidth',2);
plot(a,virial_MD(end/3+1:2*end/3,2),'d','linewidth',1);
plot(a,virial_xx(end/3+1:2*end/3,1),'-','linewidth',2);
plot(a,virial_MD(2*end/3+1:end,2),'s','linewidth',1);
plot(a,virial_xx(2*end/3+1:end,1),'--','linewidth',2);
